		...
		<div class="owl-carousel tyche-product-slider owl-loaded owl-drag"
			...
			<div class="owl-stage-outer">
				<div class="owl-stage"
					...
					<c:forEach var="food" items="${listFoodPagin}" >
							...
							<div class="tyche-product orange">
								<div class="tyche-product-image"
									...
									<img style="width:255px; height:320px"
										src="<%=request.getContextPath() %>/image/<c:out value= "${food.getImage() }"></c:out>"								
								...
								<div class="tyche-product-body">
									<h3>
										<a href="/CatStore/singleproduct?id=${food.getId() }"
											class="woocommerce-LoopProduct-link woocommerce-loop-product__link">${ food.getName() }</a>
									</h3>
									
									<span class="price"><span
										class="woocommerce-Price-amount amount"><fmt:formatNumber pattern="#,###,###,###"> ${food.getPrice()}</fmt:formatNumber> VNĐ</span></span> <a
										rel="nofollow"
										href="/CatStore/cart?action=add&id=${food.getId() }"
				...
				<div aria-label="Page navigation">
					<ul class="pagination pagination-lg" id="food_pagination">

						<c:if test="${pageFoodIndex == 1 }">
							<li class="page-item disabled" id="1"><a class="page-link"
								aria-label="Previous"> <span aria-hidden="true">&laquo;</span>
							</a></li>
						</c:if>
						<c:if test="${pageFoodIndex > 1 }">
							<li class="page-item " id="${pageFoodIndex-1 }"><a class="page-link"
								aria-label="Previous"> <span aria-hidden="true">&laquo;</span>
							</a></li>
						</c:if>
						<c:forEach var="i" begin="1" end="${totalFoodPage}">
							<c:choose>
								<c:when test="${i == pageFoodIndex}">
						...
						<c:choose>
							<c:when test="${pageFoodIndex == totalFoodPage }">
